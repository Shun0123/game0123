...
@bot.command()
async def 도움(ctx):
    embed = discord.Embed(title = "Game 0123", description = "만능 봇이 될 예정", color = 0x6E17E3) 
    ...
    embed.add_field(name = bot.command_prefix + "도박 [돈]", value = "일정량의 돈을 걸고 도박을 합니다", inline = False)
    embed.add_field(name = bot.command_prefix + "회원가입", value = "각종 컨텐츠를 즐기기 위한 회원가입을 합니다", inline = False)
    ...
    embed.add_field(name = bot.command_prefix + "랭킹", value = "현 랭킹을 볼 수 있습니다.", inline = False)
    embed.add_field(name = bot.command_prefix + "(히든 명령어)", value = "이 명령어를 사용하면 자신의 돈이나 경험치를 증가시킬수 있습니다.", inline = False)
    embed.add_field(name = bot.command_prefix + "만든 놈 ", value = "션.", inline = False)
